...
const allCart = document.getElementById("fullCarinho")
allCart.addEventListener('click', carregarProdutosCarinho())


function carregarProdutosCarinho(pod) {
    const section = document.getElementById("cesto");
    section.innerHTML = "";
    produtos.forEach(prad => {
        section.append(addToCart(prad.id));
    });
}
    ...
    botaoComprar.onclick = function () {
        ...
        fetch("https://deisishop.pythonanywhere.com/buy/", {
            ...
            .then(data => {
                ...
                referenciaPagamento.textContent = `Referência de pagamento: ${data.reference}   e a mensagem: ${data.message} `;
            })
            .catch(error => {
